...
namespace TrueSync {
    ...
    public sealed class TSMath {
        ...
        **/
        ...
        /// <summary>
        /// Moves the value towards the goal value with the specified limit.
        /// </summary>
        public static FP MoveTowards(FP value, FP goal, FP max)
        {
            FP diff = goal - value;
            if (Abs(diff) <= max)
                return goal;
            return value + Sign(diff) * max;
        }
        ...
        public static FP InverseLerp(FP value1, FP value2, FP amount, FP defaultValue)
        {
            if (value1 == value2)
                return defaultValue;
